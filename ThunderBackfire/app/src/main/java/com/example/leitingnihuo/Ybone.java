...
public class Ybone implements MyInterface {
    ...
    public Bitmap getBitmap() {
        this.x = myPlane.getX() - width;
        this.y = myPlane.getY() + myPlane.getHeight();
        time++;
        if (time >= 10) {
            //myGameView.pool.play(myGameView.musicId.get(2),1,1,0,0,1);
            myGameView.bullets.add(new Bullet(R.mipmap.newbullet_2, 0, 0, 4, myGameView));
            time = 0;
        }
        return bitmap;
    }

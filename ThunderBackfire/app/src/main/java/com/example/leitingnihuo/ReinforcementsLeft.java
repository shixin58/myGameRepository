 ...
 */
public class ReinforcementsLeft implements MyInterface {
    private MyGameView myGameView;
    private Bitmap bitmap;
    private MyPlane myPlane;
    private int x, y, width, height, time = 0, resid;

    public ReinforcementsLeft(int resid, MyGameView myGameView) {
        this.resid = resid;
    ...
    public Bitmap getBitmap() {
        this.x = myPlane.getX() - 2 * width;
        this.y = myPlane.getY() - myPlane.getHeight() / 2;
        time++;
        if (time >= 15) {
            myGameView.bullets.add(new Bullet(R.mipmap.ying, 0, 0, 2, myGameView));
            time = 0;
        }
        return bitmap;
    }

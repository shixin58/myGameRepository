...
public class QuizActivity extends FragmentActivity
        implements OnClickListener, AnimationListener, DialogInterface.OnClickListener {
    ...
    public void init() {
        in_1 = AnimationUtils.loadAnimation(QuizActivity.this, R.anim.trans1);
        in_1.setAnimationListener(this);
        in_2 = AnimationUtils.loadAnimation(QuizActivity.this, R.anim.trans3);
        in_2.setAnimationListener(this);
        out_1 = AnimationUtils.loadAnimation(QuizActivity.this, R.anim.trans2);
        out_2 = AnimationUtils.loadAnimation(QuizActivity.this, R.anim.trans4);
        ...
        switch (v.getId()) {
            case R.id.theprevious:
                if (count > 0) {
                    if (chooses[count] == null || chooses[count] == "") {
                        if (isOdd(count)) {
                            if (fragment2.flag) {
                                Toast.makeText(QuizActivity.this, "请点击确认按钮", Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(QuizActivity.this, "第" + (count + 1) + "题还没选择答案", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            if (fragment1.flag) {
                                Toast.makeText(QuizActivity.this, "请点击确认按钮", Toast.LENGTH_SHORT).show();
                    ...
                    Toast.makeText(QuizActivity.this, "已经是第一题了！！！", Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.thenext:
                if (chooses[count] == null || chooses[count] == "") {
                    if (isOdd(count)) {
                        if (fragment2.flag) {
                            Toast.makeText(QuizActivity.this, "请点击确定按钮", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(QuizActivity.this, "第" + (count + 1) + "题还没选择答案", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                    ...
                    if (count < problems.length - 2) {
                        ...
                        Toast.makeText(QuizActivity.this, "已经是最后一题了！！！", Toast.LENGTH_SHORT).show();
                    }
                }
                break;
            case R.id.queren:
                if (isOdd(count)) {
                    if (fragment2.anser.equals("")) {
                        Toast.makeText(QuizActivity.this, "请选择答案", Toast.LENGTH_SHORT).show();
                    } else {
                        IsNext = true;
            ...
            case R.id.jieshu:
                if (jieguo.getText().toString().length() >= 30) {
                    ...
                    Toast.makeText(QuizActivity.this, "至少回答5个题，才能查看结果！！！", Toast.LENGTH_SHORT).show();
                }
                break;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            Intent intenttwo = new Intent(QuizActivity.this, ExitActivity.class);
